var Demo = (function() {
	...
	function popupResult(result) {
		...
		if (result.src) {
			html = '<img src="' + result.src + '" />';
                ...
                //$('#upload-demo-result').append('<img src="' + result.src + '" />');   
                
                pushResult(result.src);
		/*swal({
			title: '',
			html: true,
			text: html,
        ...
        function pushResult(file){
            var js = JSON.stringify(file);
            $.ajax({
                type: "POST",
                url: "/member/pushimg",
                data:"file="+file,
	...
	function demoUpload() {
		...
		$('.upload-result').on('click', function (ev) {
                    //$('.upload-msg img').attr('src',$('#loading-img').val());
                    $('#ProfileImageModal .modal-footer').html('<img style="margin:auto;" class="img-fluid" src="' + $('#loading-img').val() + '" />');
                    var src = $('.upload-demo .cr-image').attr('src');
			...
			$uploadCrop.croppie('result', {
				...
				popupResult({
					src: resp
                        ...
                        //location.reload();
                        //$('.upload-msg img').attr('src',$('#old-img').val());
                        setTimeout(function(){
                            location.reload();
                        }, 500);
                    }
