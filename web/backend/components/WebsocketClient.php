...
class WebsocketClient {
    ...
    public function sendToSocket($data) {
        $port = Yii::$app->params['socket_port'];
        $sp = $this->websocket_open('localhost', $port);
        if($sp['status']){
            //$data = ['action' => 'push_socket', 'user_id' => $user->id, 'line_id' => $user->line_id, 'admin' => $admin_id, 'msg' => $msg];
            $r = $this->websocket_write($sp['msg'], json_encode($data));
            return ['status' => true, 'msg' => $r];
        }
        else{
            return $sp;
        }
    ...
    public function websocket_open($host='',$port=80,$headers='',&$error_string='',$timeout=10,$ssl=false, $persistant = false, $path = '/'){
        ...
        if(!$sp){
          ...
          return ['status' => false, 'msg' => $error_string];
        }
